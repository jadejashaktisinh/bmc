...
export default function PricingCard({ plan, price, list, btnText, popular,custom }: { plan: string, price: string, list: string[], btnText: string, popular?: boolean, custom?: boolean }) {
  return (
    <>
      <div className={`bg-[${popular ? '#FFFFFF' : '#F1F4F7'}] p-10 rounded-3xl flex flex-col justify-between ${popular ? 'h-[110%] border-[3px] border-solid border-[#000B60] relative' : ''}`}>
        <div>
          ...
          <p className="text-3xl leading-10 mb-8">{price}<span className="text-base leading-6">{custom ? '' : '/mo'}</span></p>
        </div>
